...
const mugBody = new THREE.Mesh(
  new THREE.CylinderGeometry(0.1, 0.1, 0.15, 32),
  whiteMaterial,
);

const mugHandle = new THREE.Mesh(
  new THREE.TorusGeometry(0.05, 0.015, 16, 32, Math.PI),
  whiteMaterial,
);
...
mugGroup.position.set(-0.8, 1.55, 1.2);
...
artGroup.position.set(-1.5, 3, -3.9);
...
for (let i = 0; i < 5; i++) {
  ...
  line.position.set(-1.5, 3 + i * 0.08 - 0.15, -3.82);
...
clockMesh.position.set(2.5, 3.2, -3.85);
